...
def fetch_property_and_comps(address):
    """Fetch subject property AND comps in ONE API call - works for ANY address (listed or not)"""
    ...
    # STEP 1: Get subject property using aknahin/zillow-property-info-scraper
    subject_input = {
        "addresses": address
    }
    
    try:
        # Start subject property fetch
        ...
        if response.status_code != 201:
            print(f"❌ Subject property fetch error: {response.status_code}")
            return None
        
        run_data = response.json()
        run_id = run_data['data']['id']
        print(f"⏳ Fetching subject property...")
        ...
        for i in range(60):
            ...
            if status == 'SUCCEEDED':
                ...
                if properties and len(properties) > 0:
                    ...
                    print(f"✅ Subject: {subject_property['beds']}bd/{subject_property['baths']}ba, {subject_property['sqft']}sqft")
                    print(f"💰 Zestimate: ${subject_property['zestimate']:,}")
                    print(f"🏘️ Rent Zestimate: ${subject_property['rent_zestimate']:,}/mo")
                    break
                    
            elif status == 'FAILED':
                print("❌ Subject property fetch failed")
                return None
        
        if not subject_property:
            print("⏱️  Timeout getting subject property")
            return None
        
        # STEP 2: Get comps using subject property data
        print(f"🔍 Fetching comps for {subject_property['zipcode']}...")
        ...
        if comps_response.status_code != 201:
            print(f"❌ Comps fetch error: {comps_response.status_code}")
            return {
        ...
        for i in range(60):
            ...
            if status == 'SUCCEEDED':
                ...
                print(f"✅ Found {len(comps)} comps within 2 miles")
                break
                
            elif status == 'FAILED':
                print("❌ Comps fetch failed")
                break
...
def analyze_property():
    ...
    try:
        ...
        print(f"🏘️ Rent Zestimate: ${property_data['rent_zestimate']:,}")
